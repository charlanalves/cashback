...
QueryBuilder.prototype.ucfirst = function(str) {
	  str += ''
	  var f = str.charAt(0)
	    .toUpperCase()
...
QueryBuilder.prototype.getHtmlOptionField = function(field) {

	if (field.htmlOptions === 'undefined' || field.htmlOptions == '') {
		return '';
	}

	var attributes = '';

    for(var prop in field.htmlOptions) {
    	attributes += prop+'="'+field.htmlOptions[prop]+'"';
	  }

    return attributes;
}

QueryBuilder.prototype.getHtmlOptions = function(field) {
...
QueryBuilder.prototype.getClassElement = function(field) {

	if (typeof field.htmlOptions === 'undefined' || typeof field.htmlOptions.class === 'undefined' || field.htmlOptions.class == '') {
		return '';
	}


    return field.htmlOptions.class;
}


QueryBuilder.prototype.getIconField = function(field) {

	if (typeof field.iconClass === 'undefined' || field.iconClass == '') {
		return '';
	}

    return '<i class="'+field.iconClass+'"></i>';
}
QueryBuilder.prototype.getTableByField = function(field) {

	if (typeof field.parent === 'undefined' || field.parent == '') {
		return '';
	}

    return $('#'+field.parent+field.nrel+' option:selected').text();
}

QueryBuilder.prototype._getHtmlOptionsSelect = function(field) {
	var elementOptions = '';
	var that = this;
	if (typeof field.parent !== 'undefined' && field.parent != '') {

		if (typeof field.customRuleBlockField !== 'undefined' && field.customRuleBlockField == true) {

				var valPc = $('#'+field.parentChild+field.nrel).val();

				$.each(field.data, function(k, v){
					if (v.value != valPc || v.value == 0) {
						elementOptions += '<option table="'+that.getTableByField(field)+'" disabled '+that.getHtmlOptions(v)+' value="'+v.value+'">'+v.text+'</option>';

					} else {
						elementOptions += '<option table="'+that.getTableByField(field)+'" '+that.getHtmlOptions(v)+' value="'+v.value+'">'+v.text+'</option>';
					}
				});
...
QueryBuilder.prototype.setHtmlOptionsSelect = function(field) {

	var selectFirstOption = this.getFirstOptionSelect(field);

	$('#'+field.id+field.nrel).empty().append(selectFirstOption+field.data);

	delete field.data;
}

QueryBuilder.prototype.disableSelectBootstrap = function(field, disabled) {
	$('#'+field.id+field.nrel).prop('disabled', disabled);
	$('#'+field.id+field.nrel).selectpicker('refresh');
}

QueryBuilder.prototype.getFirstOptionSelect = function(field) {
	var elementText = '';

	if (typeof field.text != 'undefined' && field.text != '') {
		 elementText = '<option value="0">'+field.text+'</option>';
	}

	return elementText;
}

QueryBuilder.prototype._fillSelect = function() {
	var selectOptions = '';
	selectOptions = '<option value="0">Selecione a tabela:</option>';

	$('select.tbp option:selected').each(function() {
	   var val = $(this).val();
	...
	if (this.settings.newRule) {
		var newSelectTb = $('#'+this.settings.ruleId).find('select.tbc:first');
		this.settings.newRule = false;
	} else{
		var newSelectTb = 'select.tbc';
	}

	$(newSelectTb).selectpicker('destroy');
	$(newSelectTb)
	    .find('option')
	    .remove()
	    .end()
	    .append(selectOptions);
	$(newSelectTb).selectpicker('render');

	//desabilita todos 'selects' de colunas cujo a tabela ainda não foi selecionada
	$('select.tbc').each(function(){
		  if ($(this).val() == 0) {
		      var selectCol = '#'+$(this).closest('li').find('select.colc').attr('id');
		       $(selectCol).val(0);
			   $(selectCol).prop('disabled', true);
			   $(selectCol).selectpicker('refresh');
		       $(selectCol).attr('charlan','teste');
		   }
		})
...
QueryBuilder.prototype.fillSelect = function() {
	var that = this;
	$('#relacoes').on('change','select.tbp', function(){
		 that._fillSelect()
	 });
}
...
QueryBuilder.prototype.getHtmlOptionsSelect = function(field) {

	var elementOptions = '';
	var that = this;
	if (typeof field.parent !== 'undefined' && field.parent != '' && typeof field.callback !== 'undefined' && field.parent != '') {

		$('#'+this.settings.divMainId).on('change', '#'+field.parent+field.nrel, function() {
			if ($(this).val() != 0) {
				field.nrel = $(this).closest('li').attr('id');
				field.data = field.callback($(this).val());
				field.data = that._getHtmlOptionsSelect(field);
				that.setHtmlOptionsSelect(field);
				that.disableSelectBootstrap(field, false);

			} else {
				that.disableSelectBootstrap(field, true);
			}
		});
	...
	return this._getHtmlOptionsSelect(field);
}


QueryBuilder.prototype.getHtmlText = function(field) {
	field.nrel = this.settings.ruleId;
	var idEl = field.id+field.nrel;
	var elementStart = '<div style="float:right;">';

	var elementText = '<input type="text" id="'+idEl+'" class=" '+this.getClassElement(field)+'" '+this.getHtmlOptionField(field)+' />';
	var elementOptions = ''

	var elementEnd = '</div>';
	var element = elementStart+elementText+elementOptions+elementEnd;

	this.settings.customFields.push(element);
}

QueryBuilder.prototype.getHtmlSelect = function(field) {
	field.nrel = this.settings.ruleId;
	var idEl = field.id+field.nrel;
	var elementStart = '<select id="'+idEl+'" class="selectpickers '+this.getClassElement(field)+'" '+this.getHtmlOptionField(field)+'>';

	var elementText = this.getFirstOptionSelect(field);
	var elementOptions = this.getHtmlOptionsSelect(field);

	var elementEnd = '</select>';
	var element = elementStart+elementText+elementOptions+elementEnd;

	this.settings.customFields.push(element);
}

QueryBuilder.prototype.getHtmlButton = function(field) {


	var elementStart = '<button type="button" '+this.getHtmlOptionField(field)+'>'+this.getIconField(field);
	var elementText = field.text;
	var elementEnd = '</button>';

	var element = elementStart+elementText+elementEnd;
 ...
 *
 */
QueryBuilder.prototype.getHtmlSelectTables = function() {

	var method_prefix = 'getHtml';
	var method_name = '';
	var functionName = '';
	var that = this;
	this.settings.customFields = [];

	$.each(this.settings.relations.fields, function(k, field){
		if (typeof field.type === 'undefined' || field.type == '') {
			field.type = 'select';
		}

		functionName = method_prefix + that.ucfirst(field.type)

		if (typeof that[functionName] !== 'function') {
			console.log('Erro: Você deve extender o plugin criando uma funcao nesse formato QueryBuilder.prototype.'+functionName+'(field)retornado o formato em html do tipo de campo desejado.')
			return false;
		}

		that[functionName](field);
	});
}

QueryBuilder.prototype.getSettings = function() {
...
QueryBuilder.prototype.createRuleFilters = function(rule) {
	this.settings.ruleId = rule.id;
	this.settings.newRule = $('#'+rule.id+' .rule-filter-container').is(':empty');

	if (Object.keys(this.settings.relations).length > 0 && this.getHtmlSelectTables() !== false) {
			rule.$el.find(Selectors.filter_container).prepend(this.settings.customFields);
		...
		if (this.settings.newRule) {
			if(typeof this.settings.firstFilters === 'undefined'){
				this.settings.firstFilters = this.filters;
			}

			var filters = this.change('getRuleFilters', this.settings.firstFilters, rule);
		  ...
		  rule.$el.find(Selectors.rule_filter).selectpicker('destroy');
	  	  rule.$el.find(Selectors.rule_filter).remove();
    ...
    setFilters: function(delete_orphans, filters) {
        ...
        (function updateBuilder(node) {
              ...
              if (rule.filter && filtersIds.indexOf(rule.filter.id) === -1) {
            	 ...
            	 var group = self.getModel($('#'+rule.groupid));
            	 $.each(group.rules, function(k, r){
            		 if (r.id == rule.id) {
            			 rule = r;
            			 return;
            		 }
