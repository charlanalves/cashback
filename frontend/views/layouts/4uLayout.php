...
<script>
	...
	*/
	...
	function loadLayout(){
		...
		var innerLayout = outerLayout.cells("a").attachLayout("4U");

		// setando o titulo do bar de filtro
        //innerLayout.cells("a").setText("Filtro<img src='<?=\Yii::getAlias('@assetsPath');?>/layoutMask/imgs/filtro_icon.png' style='width: 21px; margin-top: 0px; margin-right: 7px; float: left' text='Filtro' alt='Filtro'/>");
        //innerLayout.setCollapsedText("a", "Filtro<img src='<?=\Yii::getAlias('@assetsPath');?>/layoutMask/imgs/filtro_icon.png' style='width: 13px;  margin-top: 0px; margin-right: 7px; float: left' text='Filtro' alt='Filtro'/>");

		outerLayout.cells("a").hideHeader();
		innerLayout.cells("a").hideHeader();
		innerLayout.cells("b").hideHeader();
		innerLayout.cells("c").hideHeader();
		innerLayout.cells("d").hideHeader();
		...
		return{
			...
			telaCentral  : innerLayout.cells("b"),
			telaDir   : innerLayout.cells("c"),
			...
			tamanho: function(tamanhoA,tamanhoB,tamanhoC,tamanhoD){
                ...
                innerLayout.cells("d").setWidth(tamanhoD);

			},
			altura: function(tamanhoA,tamanhoB,tamanhoC,tamanhoD){
				innerLayout.cells("a").setHeight(tamanhoA);
				innerLayout.cells("b").setHeight(tamanhoB);
                innerLayout.cells("c").setHeight(tamanhoC);
                innerLayout.cells("d").setHeight(tamanhoD);
            ...
            t4: function(titulo){
                innerLayout.cells("d").showHeader();
				innerLayout.cells("d").setText(titulo);
				innerLayout.setCollapsedText("d", titulo);
			},
		}
	}
