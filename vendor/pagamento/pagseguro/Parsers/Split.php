...
namespace vendor\pagamento\pagseguro\Parsers;

use vendor\pagamento\pagseguro\Domains\Requests\Requests;

/**
 * Class Basic
 * @package vendor\pagamento\pagseguro\Parsers
 */
trait Split
{
    /**
                ...
                foreach ($receivers as $key => $value) {
                    ...
                    if (!is_null($receivers[$key]->getAmount())) {
                        $data[sprintf($properties::RECEIVER_SPLIT_AMOUNT, $count)] =
                            \vendor\pagamento\pagseguro\Helpers\Currency::toDecimal($receivers[$key]->getAmount());
                    }
                    if (!is_null($receivers[$key]->getRatePercent())) {
                        $data[sprintf($properties::RECEIVER_SPLIT_RATE_PERCENT, $count)] =
                            \vendor\pagamento\pagseguro\Helpers\Currency::toDecimal($receivers[$key]->getRatePercent());
                    }
                    if (!is_null($receivers[$key]->getFeePercent())) {
                        $data[sprintf($properties::RECEIVER_SPLIT_FEE_PERCENT, $count)] =
                            \vendor\pagamento\pagseguro\Helpers\Currency::toDecimal($receivers[$key]->getFeePercent());
                    }
                }
            }
        }
