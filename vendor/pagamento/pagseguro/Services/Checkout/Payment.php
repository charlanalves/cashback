...
namespace vendor\pagamento\pagseguro\Services\Checkout;
...
class Payment
{
     ...
     * @throws \Exception
     */
    public static function checkout(Credentials $credentials, \vendor\pagamento\pagseguro\Domains\Requests\Payment $payment, $onlyCode)
    {
        ...
        try {
            ...
            Logger::info(
                sprintf(
                    "Params: %s",
                    json_encode(Crypto::encrypt(\vendor\pagamento\pagseguro\Parsers\Checkout\Request::getData($payment)))
                ),
                ['service' => 'Checkout']
            );
            $http->post(
                self::request($connection),
                \vendor\pagamento\pagseguro\Parsers\Checkout\Request::getData($payment),
                20,
                \vendor\pagamento\pagseguro\Configuration\Configure::getCharset()->getEncoding()
            );

            $response = Responsibility::http(
                $http,
                new \vendor\pagamento\pagseguro\Parsers\Checkout\Request
            );
