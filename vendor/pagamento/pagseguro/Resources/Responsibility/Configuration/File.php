...
use vendor\pagamento\pagseguro\Configuration\Wrapper;
...
class File implements Handler
{
    ...
    public function handler($action, $class)
    {
        if (file_exists(PS_CONFIG_PATH."Wrapper.php")) {
            $wrapper = new Wrapper;
            return array_merge(
                \vendor\pagamento\pagseguro\Helpers\Wrapper::environment($wrapper),
                \vendor\pagamento\pagseguro\Helpers\Wrapper::credentials($wrapper),
                \vendor\pagamento\pagseguro\Helpers\Wrapper::charset($wrapper),
                \vendor\pagamento\pagseguro\Helpers\Wrapper::log($wrapper)
            );
        }
