...
order by `cashback`.`CB07_CASH_BACK`.`CB07_PERCENTUAL` desc
	...
	select 
		PAG04_TRANSFERENCIAS.PAG04_ID_PEDIDO AS PEDIDO_ID,
		date_format(T_C2E.PAG04_DATA_CRIACAO,'%m/%d/%Y %H:%i') AS DATA_OPERACAO,
		T_C2E.PAG04_VLR AS VALOR_VENDA,
		T_E2ADQ.PAG04_VLR AS TAXA_ADQUIRENTE,
		T_E2ADM.PAG04_VLR AS TAXA_ESTALECA,
		T_E2M.PAG04_VLR AS DINHEIRO_VOLTA,
		(T_C2E.PAG04_VLR - (T_E2ADQ.PAG04_VLR + T_E2ADM.PAG04_VLR + T_E2M.PAG04_VLR)) AS SALDO,
		date_format(T_C2E.PAG04_DT_DEP,'%m/%d/%Y %H:%i') AS DATA_LIBERACAO,
		PAG04_TRANSFERENCIAS.PAG04_ID_USER_CONTA_ORIGEM AS USER_ORIGEM,
		PAG04_TRANSFERENCIAS.PAG04_ID_USER_CONTA_DESTINO AS USER_DESTINO

	from PAG04_TRANSFERENCIAS 
		LEFT JOIN PAG04_TRANSFERENCIAS T_C2E on(PAG04_TRANSFERENCIAS.PAG04_ID_PEDIDO = T_C2E.PAG04_ID_PEDIDO AND T_C2E.PAG04_TIPO = 'C2E')
		LEFT JOIN PAG04_TRANSFERENCIAS T_E2ADQ on(PAG04_TRANSFERENCIAS.PAG04_ID_PEDIDO = T_E2ADQ.PAG04_ID_PEDIDO AND T_E2ADQ.PAG04_TIPO = 'E2ADQ')
		LEFT JOIN PAG04_TRANSFERENCIAS T_E2ADM on(PAG04_TRANSFERENCIAS.PAG04_ID_PEDIDO = T_E2ADM.PAG04_ID_PEDIDO AND T_E2ADM.PAG04_TIPO = 'E2ADM')
		LEFT JOIN PAG04_TRANSFERENCIAS T_E2M on(PAG04_TRANSFERENCIAS.PAG04_ID_PEDIDO = T_E2M.PAG04_ID_PEDIDO AND T_E2M.PAG04_TIPO = 'E2M')
	where 
		T_C2E.PAG04_DATA_CRIACAO IS NOT NULL AND PAG04_TRANSFERENCIAS.PAG04_TIPO IN('C2E','E2ADQ','E2ADM','E2M') 
		GROUP BY PAG04_TRANSFERENCIAS.PAG04_ID_PEDIDO;



SELECT SUM(VIEW_EXTRATO.VALOR) AS SALDO 
FROM VIEW_EXTRATO 
-- PEDIDOS DEPOSITADOS
INNER JOIN (
	SELECT PEDIDO_ID 
	FROM VIEW_EXTRATO 
	WHERE TIPO = 'E_C2E' AND DT_DEPOSITO IS NOT NULL
...
--VIEW_EXTRATO_ESTABELECIMENTO
ALTER ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `VIEW_EXTRATO_ESTABELECIMENTO` AS select `PAG04_TRANSFERENCIAS`.`PAG04_ID_PEDIDO` AS `PEDIDO_ID`,date_format(`T_C2E`.`PAG04_DATA_CRIACAO`,'%m/%d/%Y %H:%i') AS `DATA_OPERACAO`,`T_C2E`.`PAG04_VLR` AS `VALOR_VENDA`,`T_E2ADQ`.`PAG04_VLR` AS `TAXA_ADQUIRENTE`,`T_E2ADM`.`PAG04_VLR` AS `TAXA_ESTALECA`,`T_E2M`.`PAG04_VLR` AS `DINHEIRO_VOLTA`,(`T_C2E`.`PAG04_VLR` - ((`T_E2ADQ`.`PAG04_VLR` + `T_E2ADM`.`PAG04_VLR`) + `T_E2M`.`PAG04_VLR`)) AS `SALDO`,date_format(`T_C2E`.`PAG04_DT_DEP`,'%m/%d/%Y %H:%i') AS `DATA_LIBERACAO` from ((((`PAG04_TRANSFERENCIAS` left join `PAG04_TRANSFERENCIAS` `T_C2E` on(((`PAG04_TRANSFERENCIAS`.`PAG04_ID_PEDIDO` = `T_C2E`.`PAG04_ID_PEDIDO`) and (`T_C2E`.`PAG04_TIPO` = 'C2E')))) left join `PAG04_TRANSFERENCIAS` `T_E2ADQ` on(((`PAG04_TRANSFERENCIAS`.`PAG04_ID_PEDIDO` = `T_E2ADQ`.`PAG04_ID_PEDIDO`) and (`T_E2ADQ`.`PAG04_TIPO` = 'E2ADQ')))) left join `PAG04_TRANSFERENCIAS` `T_E2ADM` on(((`PAG04_TRANSFERENCIAS`.`PAG04_ID_PEDIDO` = `T_E2ADM`.`PAG04_ID_PEDIDO`) and (`T_E2ADM`.`PAG04_TIPO` = 'E2ADM')))) left join `PAG04_TRANSFERENCIAS` `T_E2M` on(((`PAG04_TRANSFERENCIAS`.`PAG04_ID_PEDIDO` = `T_E2M`.`PAG04_ID_PEDIDO`) and (`T_E2M`.`PAG04_TIPO` = 'E2M')))) where (`PAG04_TRANSFERENCIAS`.`PAG04_TIPO` in ('C2E','E2ADQ','E2ADM','E2M')) group by `PAG04_TRANSFERENCIAS`.`PAG04_ID_PEDIDO`
