...
class CB06VARIACAO extends BaseCB06VARIACAO
{
    ...
    public static function getPromocao($url, $filter)
    {
      ...
      $sql = "  SELECT  
                    MAX(CB07_CASH_BACK.CB07_PERCENTUAL) AS CB06_DINHEIRO_VOLTA,
                    CB06_ID, 
		...
		LEFT JOIN CB06_VARIACAO ON (CB05_ATIVO = 1 AND CB05_PRODUTO.CB05_ID = CB06_VARIACAO.CB06_PRODUTO_ID)
                ...
                WHERE CB04_EMPRESA.CB04_STATUS = 1 AND CB04_TIPO = 1 " . (!$filterBindCat ? "" : " AND CB04_CATEGORIA_ID = :categoria") . "
    ...
    /**
     * @inheritdoc
     * @return promocoes ativas do estabelecimento
     */
    public static function getPromocaoByEstabelecimento($estabelecimento)
    {
      $sql = "  SELECT  
                    CB06_ID as ID, 
                ...
                ORDER BY CB05_TITULO, CB06_DESCRICAO";
			
        $command = \Yii::$app->db->createCommand($sql);
        $command->bindValue(':estabelecimento', $estabelecimento);
